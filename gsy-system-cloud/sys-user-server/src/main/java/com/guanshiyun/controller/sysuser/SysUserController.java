...
public class SysUserController {
    ...
    public Mono<ResultT<Long>> deleteUserById(@PathVariable Long id) {
        return sysUserService.deleteUserById(id)
                ...
                .switchIfEmpty(Mono.just(ResultT.error("删除用户失败")))
                .onErrorResume(throwable -> {
                    log.error("删除用户失败", throwable);
                    return Mono.just(ResultT.error("删除用户失败 " + throwable.getMessage()));
                });
    }
    ...
    public Mono<ResultT<Long>> deleteUserByIds(@RequestBody Collection<Long> ids) {
        return sysUserService.deleteUserByIds(ids)
                .map(ResultT::success)
                .switchIfEmpty(Mono.just(ResultT.error("批量删除用户失败")))
                .onErrorResume(throwable -> {
                    log.error("批量删除用户失败", throwable);
                    return Mono.just(ResultT.error("批量删除用户失败 " + throwable.getMessage()));
                });
    }
    ...
    public Mono<ResultT<SysUserVO>> findById() {
        return Mono.deferContextual(contextView -> {
            if (!contextView.hasKey(ThreadSecurityLocalKey.THREAD_SECURITY_LOCAL_USER_ID_KEY))
                return Mono.just(ResultT.error(HttpStatus.NOT_FOUND.value(),"用户不存在" ));
            ...
            return sysUserService.findById(id)
                    ...
                    .onErrorResume(throwable -> {
                        ...
                        return Mono.just(ResultT.error("获取用户失败 " + throwable.getMessage()));
                    });
        });
    }
    ...
    public Mono<ResultT<Long>> updateUserById(@RequestBody SysUserVO sysUser) {
        return sysUserService.updateUserById(sysUser)
                .map(ResultT::success)
                .switchIfEmpty(Mono.just(ResultT.error("修改用户失败")))
                .onErrorResume(throwable -> {
                    log.error("修改用户失败", throwable);
                    return Mono.just(ResultT.error("修改用户失败 " + throwable.getMessage()));
                });
    }
    ...
    public Mono<ResultT<Long>> save(@RequestBody(required = false) SysUserSaveVO sysUserSaveVO) {
        return sysUserService.save(sysUserSaveVO)
                .map(ResultT::success)
                .switchIfEmpty(Mono.just(ResultT.error("添加用户失败")))
                .onErrorResume(throwable -> {
                    log.error("添加用户失败", throwable);
                    return Mono.just(ResultT.error("添加用户失败 " + throwable.getMessage()));
                });
    }
    ...
    public Mono<ResultT<PageResultT<List<SysUserVO>>>> findPage(
        ...
        return sysUserService.findPage(requestPage)
                ...
                .onErrorResume(throwable -> {
                    ...
                    return Mono.just(ResultT.error("获取用户列表失败 " + throwable.getMessage()));
                });
    }
